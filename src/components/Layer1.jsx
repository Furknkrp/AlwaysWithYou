    ...
    <div style={{ position: 'fixed', bottom: -10, left: 0, width: '100%', height: '100px', pointerEvents: 'none', zIndex: 3, display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
      {hills.map((h, i) => (
        <div
          key={i}
          style={{
            background: 'white', position: 'absolute', width: `${h.width}%`, height: `${h.height}px`,
            left: `${h.left}%`, borderTopLeftRadius: h.borderTopLeftRadius, borderTopRightRadius: h.borderTopRightRadius,
            filter: 'blur(12px)', opacity: h.opacity, boxShadow: '0 0 20px white',
          }}
        />
      ))}
    </div>
...
const Layer1 = ({ onUnlock }) => {
  ...
  // KRİTİK: Dosya yollarından /public/ kısmını kaldırdık!
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center overflow-hidden"
      ...
      style={{ background: 'radial-gradient(circle at center, #3d0202 0%, #000 100%)', position: 'relative', cursor: 'default' }}
    >
      <audio ref={audioRef} src={musicUrl} loop />

      <AnimatePresence>
        {trail.map((point) => (
          <motion.div key={point.id} initial={{ opacity: 1, scale: 0.5, x: "-50%", y: "-50%" }}
            animate={{ opacity: 0, scale: 1.8 }} exit={{ opacity: 0 }}
            style={{ position: 'fixed', left: point.x, top: point.y, pointerEvents: 'none', zIndex: 9999, fontSize: point.size, color: '#D4AF37', textShadow: '0 0 10px gold' }}
          >
            {point.emoji}
          </motion.div>
        ))}
      ...
      {/* ✅ BURAYA DİKKAT: Math.random kullanan her şeyi ClientOnly içine aldık */}
      <ClientOnly>
        <Snowflakes />
        <SnowAccumulation />
      </ClientOnly>
      ...
      {isStarted && (
        <motion.div initial={{ x: '-30vw', y: '80vh', rotate: -15 }} animate={{ x: '120vw', y: '10vh', rotate: -5 }} 
      ...
      <AnimatePresence mode="wait">
        {!isStarted ? (
          <motion.button key="start" onClick={startExperience} initial={{ scale: 0 }} animate={{ scale: [1, 1.2, 1], rotate: [0, 10, -10, 0] }}
            exit={{ scale: 0, opacity: 0 }} transition={{ repeat: Infinity, duration: 3 }}
            style={{ background: 'none', border: 'none', fontSize: '120px', cursor: 'pointer', zIndex: 100, filter: 'drop-shadow(0 0 20px #D4AF37)' }}
          >
            ❄️
          </motion.button>
        ) : (
          <motion.div key="card" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}
            style={{ background: 'linear-gradient(135deg, rgba(74, 4, 4, 0.95) 0%, rgba(26, 2, 2, 0.98) 100%)', padding: '60px 40px', borderRadius: '40px', border: '3px solid #D4AF37', boxShadow: '0 20px 50px rgba(0,0,0,0.6)', textAlign: 'center', width: '400px', position: 'relative', zIndex: 10 }}
          >
            <motion.div animate={{ opacity: isHovered ? 0 : 1 }} transition={{ duration: 1 }}
              style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(255, 255, 255, 0.15)', backdropFilter: 'blur(30px)', WebkitBackdropFilter: 'blur(30px)', borderRadius: '35px', zIndex: 100, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
               <span style={{ color: 'white', opacity: 0.9, fontSize: '15px', fontWeight: 'bold' }}>Buğuyu silmek için dokun... ❄️</span>
            </motion.div>
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <motion.input type="text" placeholder="Özel şifremiz..." value={password} onChange={(e) => setPassword(e.target.value)}
